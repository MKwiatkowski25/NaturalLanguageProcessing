...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc = min(len(document), np.random.randint(4, int(self.block_size * 7 / 8) + 1))

        document_truncated = document[:trunc]
        length = np.random.binomial(trunc, 1 / 4)
        start = max(0, int(np.random.normal((trunc - length)/2, (trunc - length)/6)))
        prefix = document_truncated[:start]
        masked_content = document_truncated[start:(start+length)]
        suffix = document_truncated[(start+length):]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + \
                        (self.block_size-trunc-1) * self.PAD_CHAR
